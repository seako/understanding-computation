...
class BinOp
  def evaluate(environment)
    self.class.value_klass.new(left.evaluate(environment).value.send(op, right.evaluate(environment).value))
  end
end

class UnaryOp
  def evaluate(environment)
    self.class.value_klass.new(expression.evaluate(environment).send(op))
  end
end
